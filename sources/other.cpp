...
const std::string kDefaultMemberTemplate = R"(member("{SHORT_MEMBER_NAME}", &{MEMBER_NAME}))";

// Specialization template data
struct Struct
{
    std::string name;
...
class StructDeclASTVisitor : public clang::RecursiveASTVisitor<StructDeclASTVisitor>
{
  ...
  public:
    ...
    bool VisitCXXRecordDecl(clang::CXXRecordDecl * decl)
    {
        ...
        {
            Struct s;
            s.name = decl->getQualifiedNameAsString(); // type

            const auto  fields = decl->fields();
            std::string buff;

            std::for_each(std::begin(fields), std::end(fields), [&buff](const auto & f) {
                using namespace fmt::literals;
                const auto shortMemberName = f->getNameAsString();
                const auto memberName      = f->getQualifiedNameAsString();
                buff += "        ";
                buff += fmt::format(kDefaultMemberTemplate, "SHORT_MEMBER_NAME"_a = shortMemberName,
...
static llvm::cl::opt<std::string> option{"template-file", llvm::cl::desc("path to template file"),
...
int main(int argc, const char * argv[])
{
    ...
    // Assemble specialization template
    std::string buff, includeBuff;
    for (const auto & oneEntry : data)
    {
        using namespace fmt;
        buff += fmt::format(kDefaultSpecializationTemplate, "TYPE"_a = oneEntry.name,
                            "META_MEMBERS"_a = oneEntry.memberTemplatesFilled);
        buff += "\n";
    }

    // Get includes of the file
    using namespace fmt;
    {
    ...
    // Assemble all
    std::cout << fmt::format(kDefaultFileTemplate, "INCLUDE_GUARD_NAME"_a = randomHeaderName(), "META_IMPL"_a = buff,
                             "INCLUDES"_a = includeBuff)
              << std::endl;
